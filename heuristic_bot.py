...
BOARD_SIZE = 20  # Kích thước bàn cờ
...
PATTERN_SCORES = {
    ...
    "0xxx1": 50,     "1xxx0": 50,
    #bot phòng thủ
    "1111x" : 20005, # ƯU TIÊN chặn nếu người chơi sắp thắng
    "x1111" : 20005,
    "0111x" : 1005,
    "x1110" : 1005,
    "x0111" : 1005,
...
class HeuristicBot:
    ...
    def evaluate_move(self, board, x, y, player, opponent):
        ...
        score += self.block_opponent_long_chains(board, x, y, player, opponent)
    ...
    def block_opponent_long_chains(self,board, x, y, player, opponent):
        ...
        for dx, dy in DIRECTIONS:
            ...
            if count_opponent == 3 and open_ends > 0:
                score += 5000
            if count_opponent == 4 and open_ends > 0: # Nếu chuỗi đối thủ có 4 quân và ít nhất 1 đầu mở → rất nguy hiểm, ưu tiên chặn rất lớn
                score += 10000
